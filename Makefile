...
CFLAGS := -Os -c -DBOF -fno-builtin -D__USE_MINGW_ANSI_STDIO=0 -Wno-int-conversion -Wno-incompatible-pointer-types
CXXFLAGS := -Os -c -DBOF -fno-builtin -D__USE_MINGW_ANSI_STDIO=0 -Wno-conversion-null -Wno-write-strings
ENTRY_CXXFLAGS := $(CXXFLAGS) -fno-exceptions -fno-rtti -fpermissive -w
...
all: clean dist/$(BOFNAME).x64.o

# Linking stage

dist/$(BOFNAME).x64.o: $(OBJS_x64)
	@mkdir -p dist
	$(BOFLINK) --mingw64 $(OBJS_x64) --entry=go -L. -lkernel32 -lmsvcrt -lgcc -lntdll -ladvapi32 -lucrt -lgdi32 -luser32 -lws2_32 -lshell32 -lcomdlg32 -lole32 -loleaut32 -luuid -lrpcrt4 -lwininet -lcrypt32 -ldbghelp -lversion -lsetupapi -lcldapi -o $@

dist/$(BOFNAME).x86.o: $(OBJS_x86)
	@mkdir -p dist
	$(BOFLINK) --mingw32 $(OBJS_x86) --entry=go -L. -lkernel32 -lmsvcrt -lntdll -ladvapi32 -lucrt -lgdi32 -luser32 -lws2_32 -lshell32 -lcomdlg32 -lole32 -loleaut32 -luuid -lwininet -lcrypt32 -ldbghelp -lversion -lsetupapi -lcldapi -o $@

# Compilation stage for entry.c
$(ENTRY_OBJ_x64): entry.c
	@mkdir -p build
	$(CXX_x64) -o $@ $(COMINCLUDE) $(ENTRY_CXXFLAGS) -x c++ $<

$(ENTRY_OBJ_x86): entry.c
	@mkdir -p build
	$(CXX_x86) -o $@ $(COMINCLUDE) $(ENTRY_CXXFLAGS) -x c++ $<
...
# Helpers
test:
	$(CXX_x64) entry.c $(COMMON_SRCS) -g $(COMINCLUDE) $(LIBINCLUDE) -L. -lcldapi -o $(BOFNAME).x64.exe
	$(CXX_x86) entry.c $(COMMON_SRCS) -g $(COMINCLUDE) $(LIBINCLUDE) -L. -lcldapi -o $(BOFNAME).x86.exe
